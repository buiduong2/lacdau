    ...
    <CalendarHeader>
      <CalendarHeading class="flex w-full items-center justify-between gap-2">
        <Select :default-value="placeholder.month.toString()" @update:model-value="(v: any) => {
          ...
          <SelectTrigger aria-label="Chọn tháng" class="w-[60%]">
        ...
        <Select :default-value="placeholder.year.toString()" @update:model-value="(v: any) => {
          ...
          <SelectTrigger aria-label="Chọn năm" class="w-[40%]">
            <SelectValue placeholder="Chọn năm" />
          </SelectTrigger>
          <SelectContent class="max-h-[200px]">
            <SelectItem v-for="yearValue in createDecade({ dateObj: date, startIndex: -100, endIndex: 0 })"
