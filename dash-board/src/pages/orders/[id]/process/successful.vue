...
import OrderProcessDeliveredFormConfirmOrderItem from '@/components/page/orders/process/OrderProcessDeliveredFormConfirmOrderItem.vue';
...
async function onSubmit(message: string) {
  const result = await props.order.updateProcess(OrderStage.COMPLETED, message);
  if (result) {
    router.replace({ name: '/orders/[id]/process/completed', params: { id: props.order.order.value.id } })
  }
}
    ...
    <Form v-slot="{ meta, values, validate }" as="" keep-values
      ...
      <Stepper v-slot="{ isNextDisabled, isPrevDisabled, nextStep, prevStep }" v-model="stepIndex" class="block w-full">
        <form class="flex flex-col gap-6" @submit="(e) => {
          ...
          <AppFormGroup title="Yêu cầu"
            desc="Đơn hàng đã thành công. Hãy hoàn tất trạng thái cuối cùng. Sau khi thành công đơn hàng chính thức kết thúc">
